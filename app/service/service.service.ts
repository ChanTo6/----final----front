...
export class ServiceService {
  ...
  baseApiUrlOperator : string = "https://localhost:7022/api/Operator";
  baseApiUrlManager : string = "https://localhost:7022/api/Manager";


  updateProduct(product: any): Observable<any> {
    console.log(product);
    return this.http.post(`${this.baseApiUrlOperator}/EditProduct`, product);
}
  ...
  removeProduct(barcode: string, quantity: number): Observable<any> {
    ...
    if (userId) {
      ...
      console.log(body);
      return this.http.post<any>(`${this.baseApiUrlOperator}/RemoveProduct`, body)
        .pipe(catchError(err => {
  ...
  FetchProducts(): Observable<WareHouse[]> {
    return this.http.get<WareHouse[]>(`${this.baseApiUrlManager}/FetchProducts`);
  }

  GetRemovedProducts(): Observable<WareHouse[]> {
    return this.http.get<WareHouse[]>(`${this.baseApiUrlManager}/GetRemovedProducts`);
  }

  GetRemovedProductsByUserId(userId: number): Observable<WareHouse[]> {
    return this.http.post<WareHouse[]>(`${this.baseApiUrlOperator}/GetRemovedProductsByUserId`, userId);
  }

  FetchProductbyuserId(userId: number): Observable<WareHouse[]> {
    return this.http.get<WareHouse[]>(`${this.baseApiUrlOperator}/FetchProductbyuserId/${userId}`).pipe(
      catchError((error) => {
        if (error.status !== 404) {
  ...
  addProduct(product: Product): Observable<any> {
    ...
    return this.http.post<any>(`${this.baseApiUrlOperator}/AddProduct`, product, { headers });
  }
  ...
  addWarehouse(warehouseData: WarehouseInfo): Observable<any> {
    const url = `${this.baseApiUrlManager}/AddWarehouse`;
